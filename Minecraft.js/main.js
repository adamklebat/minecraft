...
//  أسماء أنواع بلوك (مثل grass, dirt, stone blocks
...
function renderWorld() {
    ...
    for (let r = 0; r < worldData.length; r++) {
        for (let c = 0; c < worldData[r].length; c++) {
            ...
            tile.className = "tile " + type;  // إضافة CSS class بلوك
...
// CLICK  (التعامل مع الضغط على بلوك)

function handleTileClick(e) {
    const r = e.target.dataset.row;  // الصف
    const c = e.target.dataset.col;  // العمود
    const type = TILE_TYPES[worldData[r][c]]; // نوع بلوك
    ...
    if (currentTool) {
        ...
        if (
            ...
            inventory.push(type);       // أضف بلوك للمخزون
    ...
    // 📦 Placing (إذا اللاعب يحمل بلاطة لوضعها)
        ...
        const index = TILE_TYPES.indexOf(currentBlock); // رقم بلوك
        worldData[r][c] = index;                        // ضع بلوك في 

        const i = inventory.indexOf(currentBlock);     // حذف بلوك من المخزون
...
function renderInventory() {
    ...
    inventory.forEach(item => {
        ...
        //  البلاطة المختارة إذا كانت هذه هي
...
["axe", "pickaxe", "shovel"].forEach(tool => {
    document.getElementById(tool + "-tool").onclick = () => {
        ...
        currentBlock = null;  // remove block
    };
});
